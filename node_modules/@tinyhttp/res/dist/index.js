...
import { charset, lookup } from 'es-mime-types';
    ...
    if (typeof field === 'string') {
        ...
        if (field.toLowerCase() === 'content-type') {
            ...
            if (!charsetRegExp.test(value)) {
                const charset$1 = charset(value.split(';')[0]);
                if (typeof charset$1 === 'string')
                    value += '; charset=' + charset$1.toLowerCase();
            }
        }
...
const setCookie = (req, res) => (name, value, options = {}) => {
    ...
    if (options.path == null)
        options.path = '/';
    append(res)('Set-Cookie', `${serialize(name, String(val), options)}`);
    return res;
};
...
const normalizeType = (type) => ~type.indexOf('/') ? acceptParams(type) : { value: lookup(type), params: {} };
