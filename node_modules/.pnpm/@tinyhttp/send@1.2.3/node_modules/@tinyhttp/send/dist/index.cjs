...
const createETag = (body, encoding) => {
    if (body instanceof fs.Stats) {
        return etag.eTag(body, { weak: true });
    }
    else {
        return etag.eTag(!Buffer.isBuffer(body) ? Buffer.from(body, encoding) : body, { weak: true });
    }
};
 ...
 * @param res Response
 */
const sendFile = (req, res) => (path$1, opts = {}, cb) => {
    const { root, headers = {}, encoding = 'utf-8', caching, ...options } = opts;
    if (!path.isAbsolute(path$1) && !root)
        throw new TypeError('path must be absolute');
    if (caching)
        enableCaching(res, caching);
    const filePath = root ? path.join(root, path$1) : path$1;
    const stats = fs.statSync(filePath);
    headers['Content-Encoding'] = encoding;
    headers['Last-Modified'] = stats.mtime.toUTCString();
    headers['Content-Type'] = esMimeTypes.contentType(path.extname(path$1));
